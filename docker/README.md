...
- **CPU 架构（Apple Silicon / ARM）**：[`Dockerfile.web`](Dockerfile.web) 在构建步骤里用 **`uname -m`**（`x86_64` / `aarch64`）选择对应 Godot Linux 包，与当前镜像 CPU 一致，避免 arm64 容器误装 x86_64 二进制（曾出现的 `rosetta error` / `exec format error`）。若需构建「异架构」镜像，请使用 `docker build --platform linux/amd64 ...` 等，使基础镜像与 Godot 二进制同为该架构。  
